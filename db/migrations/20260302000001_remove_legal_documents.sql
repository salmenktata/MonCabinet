-- Migration: Suppression de la couche legal_documents
-- Date: 2026-03-02
--
-- Contexte: Suppression complète de la feature legal_documents.
--   - Suppression des tables legal_documents, web_pages_documents, legal_document_amendments
--   - Retrait du filtre EXISTS legal_documents dans search_knowledge_base_hybrid()
--   - Les chunks KB existants restent intacts (knowledge_base + knowledge_base_chunks inchangés)
--   - Le RAG revient aux filtres ws.rag_enabled + kb.is_indexed + kb.rag_enabled
--
-- ROLLBACK: Réappliquer 20260228000001_rag_legal_docs_only.sql
-- =============================================================================

-- 1. Recréer search_knowledge_base_hybrid SANS le filtre legal_documents
...
CREATE OR REPLACE FUNCTION search_knowledge_base_hybrid(
  ...
  p_language text DEFAULT 'simple',
...
Fix Feb 26 v1: OR logic BM25.
Fix Feb 26 v2: bm25_pool_limit = GREATEST(limit_count*5, 100).
Fix Feb 26 v3: BM25-only weight 0.3→0.7.
Fix Feb 27 v1: Filtre rag_enabled web.
Fix Feb 27 v2: kb.rag_enabled=true.
Fix Mar 02: Suppression filtre legal_docs — retour aux filtres ws.rag_enabled + kb.rag_enabled.';

-- 2. Dropper l index de performance lié à legal_documents
DROP INDEX IF EXISTS idx_legal_docs_knowledge_base_id;

-- 3. Dropper la fonction helper search_kb_document_aware
DROP FUNCTION IF EXISTS search_kb_document_aware CASCADE;

-- 4. Supprimer la colonne authority_score ajoutée par 20260216000001_legal_documents.sql
ALTER TABLE web_sources DROP COLUMN IF EXISTS authority_score;

-- 5. Dropper les tables legal_documents (CASCADE supprime FK deps)
DROP TABLE IF EXISTS legal_document_amendments CASCADE;
DROP TABLE IF EXISTS web_pages_documents CASCADE;
