-- Migration: Ajouter filtre rag_enabled dans search_knowledge_base_hybrid
-- Date: 2026-02-27
--
-- Contexte: Après avoir défini rag_enabled=false sur les sources non-autoritaires
-- (da5ira, jibaya.tn, legislation-securite.tn, etc.), il faut que la fonction SQL
-- de recherche RAG respecte ce flag et exclue leurs chunks du retrieval.
--
-- Le filtre s'appuie sur le chemin : knowledge_base → web_pages → web_sources.rag_enabled
-- Les docs issus de legal_documents (sans web_page associée) passent toujours (sources internes = toujours fiables).
--
-- Pattern réutilisé depuis indexPendingDocuments() dans knowledge-base-service.ts :
--   NOT EXISTS (
...
--   )
--
-- Ajouté dans les 6 CTEs (vector_results + bm25_results pour OpenAI, Gemini, Ollama).
-- =============================================================================
-- ROLLBACK:
--   Réappliquer 20260226000003_fix_bm25_only_weight.sql
-- =============================================================================
  ...
  IF embedding_provider = 'openai' THEN
    ...
    vector_results AS (
      ...
      WHERE kbc.embedding_openai IS NOT NULL
        ...
        -- Filtre rag_enabled: exclure chunks de sources désactivées
        AND NOT EXISTS (
          SELECT 1 FROM web_pages wp
          JOIN web_sources ws ON wp.web_source_id = ws.id
          WHERE wp.knowledge_base_id = kb.id
...
Fix Feb 27: Filtre rag_enabled — exclure chunks de sources web désactivées (NOT EXISTS web_pages → web_sources.rag_enabled=false).
  Sources autoritaires (rag_enabled=true): 9anoun.tn, iort.gov.tn, cassation.tn.
  Sources désactivées: da5ira, jibaya.tn, legislation-securite.tn, etc.
  Docs legal_documents sans web_page associée: toujours inclus (sources internes fiables).';
