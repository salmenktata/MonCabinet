/**
 * POST /api/admin/pipeline/bulk/advance
 * Avance N documents en batch (max 100)
 * Body: { docIds: string[], notes?: string }
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { docIds, notes } = body as { docIds: string[]; notes?: string }
    ...
    const result = await bulkAdvance(docIds, session.user.id, notes)

    return NextResponse.json(result)
  } catch (error) {
    console.error('[Pipeline API] Erreur bulk advance:', error)
