  ...
  try {
    ...
    const globalStatsResult = await db.query<{
      ...
      SELECT
        ...
        (SELECT COUNT(*) FROM knowledge_base_chunks WHERE is_active = true) as total_chunks
      FROM knowledge_base
    `)
    ...
    const providerStatsResult = await db.query<{
      ...
      SELECT
        ...
        ROUND(AVG(quality_processing_time_ms)::numeric, 0) as avg_processing_time_ms,
        ROUND(
    ...
    const response = {
      ...
      providers: providerStats.map(p => ({
        ...
        avgProcessingTimeMs: p.avg_processing_time_ms || 0,
