...
function mapDossierFromDB(row: any): any {
  return {
    ...
    numero: row.numero,
    objet: row.objet,
    description: row.objet, // alias pour compatibilité
    type: row.type_procedure,
    statut: row.statut, // valeur brute DB pour compatibilité
    ...
    montantLitige: row.montant_litige,
    montant: row.montant,
    devise: row.devise,
    partieAdverse: row.partie_adverse,
    avocatAdverse: row.avocat_adverse,
    numeroRg: row.numero_rg,
    workflowEtape: row.workflow_etape_actuelle,
    notes: row.notes,
